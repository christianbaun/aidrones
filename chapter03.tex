...
Starting with ExpressLRS (ELRS) version 3.4, it is now possible to use MAVLINK to transmit telemetry data from a UAV (unmanned aerial vehicle) to ground station devices such as laptops and tablets, in real time. These telemetry streams may include parameters such as battery voltage, GPS coordinates, altitude, airspeed or ground speed, and other relevant flight-status information.
...
\begin{itemize}
\item \textsl{Minimum version of ExpressLRS 3.5 on both TX and RX modules} - If this requirement is not met, refer to Section~\ref{AbschnittExpressLRS} for instructions on updating ExpressLRS on both the transmitter and receiver.
\end{itemize}

\begin{itemize}
\item \textsl{Minimum version of Backpack version 1.5} - If an older version is installed, the Backpack must be updated to the latest compatible release.
\end{itemize}
...
\renewcommand{\deutschertitel}{TBD}
\renewcommand{\englischertitel}{Configuring ELRS TX and RX }
\makrounterabschnitt
...
Next, on the radio controller (TX), open the ExpressLRS Lua script and navigate to ExpressLRS Lua script \(\longrightarrow\) Other Devices \(\longrightarrow\) Select appropriate receiver \(\longrightarrow\) configure the serial protocol to MAVLink. This step is no longer required starting from ExpressLRS version 4, where automatic configuration is supported.
...
After configuration, power off the receiver on the UAV. Then, on the radio controller (TX), open the ExpressLRS Lua script and enter Link Mode, setting it to Normal to MAVLink mode. Finally, power on the receiver again and verify that the transmitter successfully establishes a connection with the receiver.

\colende

\renewcommand{\deutschertitel}{TBD}
\renewcommand{\englischertitel}{Configuring Backpack for Wi-Fi MAVLink Telemetry}
\makrounterabschnitt
\label{AbschnittExpressLRSMavlinkBackpackKonfiguration}

To enable wireless telemetry communication via Wi-Fi, the ExpressLRS Backpack must be configured accordingly. In the ExpressLRS Lua script, navigate to Backpack \(\longrightarrow\) Telemetry and set the mode to WiFi. This process automatically configures the Backpack, including the assignment of the UDP port (14550).
...
If the Backpack is not connected to a home Wi-Fi network, it will create a dedicated access point named similar to ExpressLRS TX Backpack XXXXXX, with the default password expresslrs. Devices connected to this access point can access telemetry data via the assigned UDP port.
...
Alternatively, if the Backpack is connected to an existing home Wi-Fi network, it will join the local network automatically. In this mode, any device connected to the same network can receive telemetry data through UDP port 14550.
