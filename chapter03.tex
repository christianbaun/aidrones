...
Next, in the Full Parameter List, search for the parameters beginning with SRx (where x is the same UART number). Set all \verb!SRx_*! parameters to 1, except for \verb!SRx_ADSB!, \verb!SRx_PARAMS!, and \verb!SRx_RAW_CTRL!, which should remain set to 0 (see Figure~\ref{AbbildungExpressLRSMavlinkArduPilotKonfiguration}). Once done reboot the flight controller and verify that the telemetry data is being received on the ground station device.
