...
Figure~\ref{AbbildungVTXConnectors} shows the video transmitter connectors on three different flight controllers. While the left and the middle flight controller supply the video transmitter directly with the voltage of the connected battery (typically between 12.0,V and 22.2,V -- see Table~\ref{TabelleAkkus}), the right flight controller provides a regulated voltage of 9,V. Directly connecting the video transmitter shown in Figure~\ref{AbbildungSpeedyBeeTX800FPVVTX} to any of these three flight controllers would damage the device. In such a case, an intermediate BEC (Battery Eliminator Circuit) is required to step down the battery voltage to 5,V. By contrast, directly connecting the video transmitter shown in Figure~\ref{AbbildungFoxeerReaperV2VTX} to any of the three flight controllers would not pose a problem.

\colger

Abbildung~\ref{AbbildungVTXConnectors} zeigt die Anschlüsse für den Videosender bei drei verschiedenen Flugcontrollern. Während der linke und der mittlere Flugcontroller den Videosender direkt mit der Spannung des angeschlossenen Akkus versorgen (typischerweise zwischen 12.0,V und 22.2,V -- siehe Tabelle~\ref{TabelleAkkus}), stellt der rechte Flugcontroller eine Spannung von 9,V bereit. Ein direkter Anschluss des in Abbildung~\ref{AbbildungSpeedyBeeTX800FPVVTX} dargestellten Videosenders an einen der drei Flugcontroller würde den Videosender beschädigen. In einem solchen Fall ist ein zwischengeschalteter BEC (Battery Eliminator Circuit) erforderlich, der die Akkuspannung auf 5,V herunterregelt. Ein direkter Anschluss des in Abbildung~\ref{AbbildungFoxeerReaperV2VTX} dargestellten Videosenders an einen der drei Flugcontroller wäre hingegen problemlos möglich.
