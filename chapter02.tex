...
\begin{paracol}{2}[]
	...
	ArduPilot is a firmware for flight controllers that supports fully autonomous operation of drones and other unmanned vehicles. It enables functions such as waypoint-based mission planning, automatic takeoff and landing, obstacle avoidance, Follow-Me modes, and geofencing. Configuration and mission planning are performed through external tools with graphical user interfaces. The firmware supports a wide range of sensors, including GPS, magnetometers, barometers, LiDAR, and cameras, to allow for precise navigation and environmental interaction.
	
	\colger
	
	ArduPilot ist eine Firmware für Flugcontroller, die den vollständig autonomen Betrieb von Drohnen und anderen unbemannten Fahrzeugen unterstützt. Sie ermöglicht Funktionen wie missionsbasierte Flugplanung mit Wegpunkten, automatischen Start und automatische Landung, Hindernisvermeidung, Follow-Me-Modi und Geofencing. Die Konfiguration und Missionsplanung erfolgen über externe Tools mit grafischen Benutzeroberflächen. Die Firmware unterstützt eine Vielzahl von Sensoren, darunter GPS, Magnetometer, Barometer, LiDAR und Kameras, um eine präzise Navigation und Umgebungswahrnehmung zu ermöglichen.
	
	\coleng
	
	Of the flight controller firmwares presented in this document, ArduPilot requires the most memory. Ideally, 2\,MB of flash memory is available for running ArduPilot. This is only provided by flight controllers with an H743 microcontroller. With 1\,MB of flash memory (F405 and F745 microcontrollers), it is possible to operate ArduPilot with a reduced feature set. With only 512\,kB (F411 and F722 microcontrollers), ArduPilot cannot be used at all.
	...
	Due to the firmware comparatively high memory and hardware requirements, it is necessary to evaluate and confirm the suitability of a specific flight controller in advance to ensure stable and complete functionality.
	
	\colger
	
	Aufgrund der vergleichsweise hohen Anforderungen an Speicher und Hardware ist es erforderlich, die Eignung eines bestimmten Flugcontrollers im Vorfeld zu prüfen, um einen stabilen und vollständigen Betrieb sicherzustellen.
	...
	\renewcommand{\deutschertitel}{Mandatory Configurations}
	\renewcommand{\englischertitel}{Mandatory Configurations}
	\makrounterunterabschnitt
