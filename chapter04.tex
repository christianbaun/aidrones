...
During flight, a sufficiently large distance from other persons must always be maintained. Buildings belonging to others and private property must not be overflown without prior permission. In any case, be sure to observe the applicable local regulatory and legal requirements (see Chapter~\ref{KapitelRegulatorischeGrundlagen}).

\colger

Während des Fluges ist stets ein möglichst großer Abstand zu anderen Personen einzuhalten. Fremde Gebäude und private Grundstücke dürfen nicht ohne vorherige Zustimmung überflogen werden. Beachten Sie auf jeden Fall die lokalen regulatorischen und gesetzlichen Grundlagen (siehe Kapitel~\ref{KapitelRegulatorischeGrundlagen}).
...
\renewcommand{\deutschertitel}{Spezifikation der Anschlüsse beachten}
\renewcommand{\englischertitel}{Observing Electrical Interface Specifications}
\makroabschnitt
\label{AbschnittHerstellerspezifischeAnschluesse}

\coleng

Although modern flight controllers often provide plug connectors that simplify the integration of external components, a basic understanding of electrical properties such as voltage, current, and polarity remains essential. In particular during the initial phase of the project, insufficient experience in this area repeatedly resulted in incorrect connections and consequently damaged components.

\colger

Moderne Flugcontroller verfügen zwar häufig über Steckverbindungen, die den Anschluss externer Komponenten erleichtern. Dennoch ist ein grundlegendes Verständnis elektrischer Eigenschaften wie Spannung, Stromstärke und Polarität unerlässlich. Insbesondere in der Anfangsphase führte fehlende Erfahrung in diesem Bereich wiederholt zu fehlerhaften Verbindungen und in der Folge zu beschädigten Bauteilen.

\coleng

A typical example is the connection of analog video transmitters. Operating a common analog video transmitter generally requires four connections: a power supply line, ground, a video signal line, and a control line used for configuration. However, flight controllers differ in the voltage levels they provide via their connector ports (see Figure~\ref{AbbildungVTXConnectors} presenting three different flight controllers) or solder pads. At the same time, the permissible operating voltages of video transmitters vary considerably between different models. Connecting components without carefully verifying their electrical specifications can therefore quickly lead to irreversible hardware damage.

\colger

Ein typisches Beispiel ist der Anschluss analoger Videosender. Für den Betrieb eines gängigen Videosenders werden in der Regel vier Leitungen benötigt: eine Spannungsversorgung, eine Masseleitung, das Videosignal sowie eine Steuerleitung zur Konfiguration. Flugcontroller unterscheiden sich jedoch darin, welche Spannungen sie über entsprechende Steckverbindungen oder über Lötpads bereitstellen. Gleichzeitig variieren die zulässigen Betriebsspannungen der Videosender je nach Modell erheblich. Werden Komponenten ohne sorgfältige Prüfung ihrer elektrischen Spezifikationen miteinander verbunden, kann dies schnell zu irreparablen Hardwareschäden führen.

\coleng

Figure~\ref{AbbildungVTXConnectors} shows the video transmitter connectors on three different flight controllers. While the left and the middle flight controller supply the video transmitter directly with the voltage of the connected battery (typically between 12.0,V and 22.2,V -- see Table~\ref{TabelleAkkus}), the right flight controller provides a regulated voltage of 9,V. Directly connecting the video transmitter shown in Figure~\ref{AbbildungSpeedyBeeTX800FPVVTX} to any of these three flight controllers would damage the device. In such a case, an intermediate BEC -- Battery Eliminator Circuit (see Figure~\ref{AbbildungBEC}) is required to step down the battery voltage to 5,V. By contrast, directly connecting the video transmitter shown in Figure~\ref{AbbildungFoxeerReaperV2VTX} to any of the three flight controllers would not pose a problem.

\colger

Abbildung~\ref{AbbildungVTXConnectors} zeigt die Anschlüsse für den Videosender bei drei verschiedenen Flugcontrollern. Während der linke und der mittlere Flugcontroller den Videosender direkt mit der Spannung des angeschlossenen Akkus versorgen (typischerweise zwischen 12.0,V und 22.2,V -- siehe Tabelle~\ref{TabelleAkkus}), stellt der rechte Flugcontroller eine Spannung von 9,V bereit. Ein direkter Anschluss des in Abbildung~\ref{AbbildungSpeedyBeeTX800FPVVTX} dargestellten Videosenders an einen der drei Flugcontroller würde den Videosender beschädigen. In einem solchen Fall ist ein zwischengeschalteter BEC -- Battery Eliminator Circuit (siehe Abbildung~\ref{AbbildungBEC}) erforderlich, der die Akkuspannung auf 5,V herunterregelt. Ein direkter Anschluss des in Abbildung~\ref{AbbildungFoxeerReaperV2VTX} dargestellten Videosenders an einen der drei Flugcontroller wäre hingegen problemlos möglich.

\colende

\begin{figure}[htb!]
  \centering
    \includegraphics[width=\linewidth]{Anschluss_VTX_Herausforderung_2026_v2.pdf}
  \caption{Improper Connection of a Video Transmitter to a Flight Controller without verifying the permitted Voltage can lead to Hardware Damage}
  \label{AbbildungVTXConnectors}
\end{figure}
\FloatBarrier

\renewcommand{\deutschertitel}{Schutz der Komponenten durch einem Smoke Stopper}
\renewcommand{\englischertitel}{Protection of the Components using a Smoke Stopper}
\makroabschnitt
\label{AbschnittSmokeStopper}

A smoke stopper (see Figure~\ref{AbbildungSmokeStopper}) is a simple protection device that is placed between the battery and the drone when the drone is connected to the battery for the first time. It typically contains a resettable fuse (PTC) or a small light bulb that limits the current flow if a short circuit or wiring error is present. This prevents very high currents from flowing immediately after the battery is connected.

\colger

Ein Smoke Stopper (siehe Abbildung~\ref{AbbildungSmokeStopper}) ist ein einfaches Schutzgerät, das beim ersten Anschluss einer Drohne an den Akku zwischen Akku und Drohne geschaltet wird. Er enthält eine selbstrückstellende Sicherung, die den Stromfluss begrenzt, wenn ein Kurzschluss oder eine fehlerhafte Verdrahtung vorliegt. Dadurch wird verhindert, dass unmittelbar nach dem Anschluss des Akkus sehr hohe Ströme fließen.

\coleng

The use of a smoke stopper is strongly recommended after assembling the drone or after making changes to its electronics. During soldering or wiring, unintended short circuits can easily occur, for example due to solder bridges, damaged cable insulation, or incorrectly connected wires. Without current limiting, such faults can destroy the flight controller, ESCs, or other electronic components within seconds. A smoke stopper limits the current so that such problems can be detected without damaging the hardware. Only after it has been verified that the drone operates correctly should it be connected directly to the battery.

\colger

Der Einsatz eines Smoke Stoppers ist besonders nach dem Zusammenbau oder nach Änderungen an der Elektronik der Drohne dringend zu empfehlen. Beim Löten oder Verdrahten können leicht unbeabsichtigte Kurzschlüsse entstehen, etwa durch Lötbrücken, beschädigte Kabelisolierungen oder falsch angeschlossene Leitungen. Ohne Strombegrenzung kann ein solcher Fehler innerhalb von Sekunden Flugcontroller, ESCs oder andere elektronische Komponenten dauerhaft beschädigen. Der Smoke Stopper begrenzt in diesem Fall den Strom so weit, dass der Fehler erkannt werden kann, ohne dass Bauteile zerstört werden. Erst wenn sichergestellt ist, dass die Drohne korrekt funktioniert, sollte sie direkt mit dem Akku betrieben werden.
...
\begin{figure}[htb]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/VIFLY_ShortSaver2_front_small.jpg}
    \vspace{0pt} % sorgt für Top-Ausrichtung
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/VIFLY_ShortSaver2_back_small.jpg}
    \vspace{0pt}
  \end{minipage}
  \caption{VIFLY ShortSaver 2 Smoke Stopper (Front and Back)}
  \label{AbbildungSmokeStopper}
\end{figure}
\FloatBarrier
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{FM2M_ToolBox_091_screenshot2.png}
    \vspace{0pt} % sorgt für Top-Ausrichtung
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{FM2M_ToolBox_091_screenshot1.png}
    \vspace{0pt}
  \end{minipage}
  \caption{FM2M ToolBox: An EdgeTX Script that aggregates Drone Sensor Data}
  \label{AbbildungFM2MScreenshots}
\end{figure}
\FloatBarrier
