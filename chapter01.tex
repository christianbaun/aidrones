...
For comparison, the flight controller shown in Figure~\ref{AbbildungFlywooFCGOKUGN745AIO}, the Flywoo GOKU GN745 AIO 45A, offers seven UART interfaces. None of them is used for USB, but two are reserved for telemetry data transmission (ESC + DJI video sender). After connecting the common components — video transmitter, receiver, and GPS module — two UART interfaces remain available for additional sensors and actuators.

\colger

Zum Vergleich: Der in Abbildung~\ref{AbbildungFlywooFCGOKUGN745AIO} gezeigte Flugcontroller Flywoo GOKU GN745 AIO 45A verfügt über sieben UART-Schnittstellen. Keine davon ist für die USB-Schnittstelle belegt, jedoch sind zwei für die Übertragung von Telemetriedaten (Motorsteuerung und DJI-Videosender) reserviert. Nach dem Anschluss der üblichen Komponenten -- Videosender, Empfänger und GPS-Modul -- bleiben somit zwei UART-Schnittstellen für weitere Sensoren und Aktoren verfügbar.
...
Flywoo GN745 AIO V3            & \cellcolor{Gray}Telem.\textsuperscript{c} & \cellcolor{Gray}Receiver & \cellcolor{Gray}VTX       & \cellcolor{green}available  & \cellcolor{green}available  & \cellcolor{Gray}GPS & \cellcolor{Gray}Telem.\textsuperscript{c} & --- \\
...
Table~\ref{TabelleMotorenuebersicht} shows typical motors for the different frame sizes.

\colger

Tabelle~\ref{TabelleMotorenuebersicht} zeigt typische Motoren für die verschiedenen Rahmengrößen.
...
\captionabove{Typical Motor Configurations for Drones}
\label{TabelleMotorenuebersicht}
\begin{tabularx}{\linewidth}{lcccX}
\toprule
Frame Size & Motor Size      & KV (4S)                          & KV (6S)                         & Typical Use \\
\midrule
2"   & 1103-1204\phantom{.0} & 9000-11000                       & ---                             & Whoop, Indoor, Micro Drones \\
2.5" & 1203-1404\phantom{.0} & 6000-8000\phantom{0}             & 4000-5500                       & Cinewhoop, Toothpick, Freestyle \\
3"   & 1204-1505\phantom{.0} & 4500-6500\phantom{0}             & 3000-4000                       & Cinewhoop, Freestyle \\
3.5" & 1404-1804\phantom{.0} & 3500-4500\phantom{0}             & 2500-3000                       & Cinewhoop, Freestyle \\
4"   & 1505-2004\phantom{.0} & 3000-4000\phantom{0}             & 2000-2500                       & Freestyle, Light Long-Range \\
5"   & 2207-2308\phantom{.0} & 2300-2700\phantom{0}             & 1600-1900                       & Freestyle \\
6"   & 2207-2507\phantom{.0} & 1700-2200\phantom{0}             & 1300-1600                       & Freestyle, Long-Range \\
7"   & 2507-2806.5           & 1300-1700\phantom{0}             & \phantom{0}900-1300             & Long-Range \\
8"+  & 2808-3110\phantom{.0} & \phantom{0}900-1300\phantom{0}   & \phantom{0}600-900\phantom{0}   & Heavy-Lift, Long-Range \\
\bottomrule
\end{tabularx}
\end{table}
\FloatBarrier
...
The propeller size (see Figure~\ref{AbbildungPropellersSize}) determines, in simple terms, how much air is moved. The specified propeller size always refers to the diameter in inches. The propellers must fit the frame used (see Section~\ref{AbschnittFrames}).

\colger

Die Propellergröße (siehe Abbildung~\ref{AbbildungPropellersSize}) bestimmt vereinfacht gesagt, wie viel Luft bewegt wird. Die angegebene Propellergröße entspricht immer dem Durchmesser in Zoll. Die Propeller müssen zum verwendeten Rahmen passen (siehe Abschnitt~\ref{AbschnittFrames}).
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{GEMFAN_51MM-4_2020_Ducted_Whisky_PC_2_Pitch_2_Inch.png}
    \vspace{0pt}
    GEMFAN 2020-4, 2 Inch Propeller Size, 2 Inch Pitch
  \end{minipage}\hfill
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{HQProp_DT90MMX5_GR_PC_Cinewhoop_Grey_3_Pitch_3_5_Inch.png}
    \vspace{0pt} % sorgt für Top-Ausrichtung   
    HQProp DT90MMX5, 3.5 Inch Propeller Size, 3 Inch Pitch 
  \end{minipage}\hfill
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{GEMFAN_Freestyle_4S_129m_Mango_Yellow_3_6_Pitch_5_1_Inch.png}
    \vspace{0pt}
    GEMFAN Freestyle 4S, 5.1 Inch Propeller Size, 3.6 Inch Pitch
  \end{minipage}
  \caption{Three Propellers of different Size}
  \label{AbbildungPropellersSize}
\end{figure}
\FloatBarrier
...
For long-range applications where efficiency is the top priority, two-blade propellers are typically used (see Figure~\ref{AbbildungTwoBladesPropeller}). Racing and freestyle drones often use three-blade propellers. CineWhoops commonly use propellers with five or more blades, as these provide maximum flight stability and precise control.

\colger

Für Long-Range-Anwendungen, bei denen Effizienz oberste Priorität hat, werden in der Regel Zweiblattpropeller verwendet (siehe Abbildung~\ref{AbbildungTwoBladesPropeller}). Racing- und Freestyle-Drohnen sind häufig mit Dreiblattpropellern ausgestattet. CineWhoops nutzen oft Propeller mit fünf oder mehr Blättern, da sie ein besonders stabiles Flugverhalten und präzise Steuerung ermöglichen.

\colende

\begin{figure}[htb]
  \centering
    \includegraphics[width=.5\linewidth]{GEMFAN_Hurricane_3018-2_Clear_1_8_Pitch_3_Inch.png}
  \caption{GEMFAN Hurricane 3018-2, 3 Inch Propeller Size 1.8 Inch Pitch}
  \label{AbbildungTwoBladesPropeller}
\end{figure}
\FloatBarrier
...
The pitch is specified in inches and describes the theoretical distance the propeller would move forward during one full rotation if there were no slippage (see Figure~\ref{AbbildungPropellersPitch}).

\colger

Der Pitch wird in Zoll angegeben und beschreibt den theoretischen Vortrieb pro Umdrehung, also wie weit sich der Propeller bei einer vollständigen Umdrehung durch die Luft schrauben würde, wenn es keine Schlupfverluste gäbe (siehe Abbildung~\ref{AbbildungPropellersPitch}).
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{GEMFAN_63MM-5_Clear_Yellow_PC_1_65_Pitch_2_5_Inch.png}
    \vspace{0pt} % sorgt für Top-Ausrichtung   
    GEMFAN D63-5, 2.5 Inch Propeller Size, 1.65 Inch Pitch 
  \end{minipage}\hfill
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{GEMFAN_Hurricane_2520-3_Whisky_2_Pitch_2_5_Inch.png}
    \vspace{0pt}
    GEMFAN 2520-3, 2.5 Inch Propeller Size, 2 Inch Pitch
  \end{minipage}\hfill
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{GEMFAN_63MM-3_Purple_PC_3_Pitch_2_5_Inch.png}
    \vspace{0pt}
    GEMFAN D63-3, 2.5 Inch Propeller Size, 3 Inch Pitch
  \end{minipage}
  \caption{Three Different Propellers of the same Size, but with different Pitch}
  \label{AbbildungPropellersPitch}
\end{figure}
\FloatBarrier
...
\renewcommand{\deutschertitel}{Optischer Fluss und LiDAR-Laserentfernungsmesser}
\renewcommand{\englischertitel}{Optical Flow and LiDAR sensor}
\makroabschnitt
\label{AbschnittOpticalFlow}

In autonomous aerial systems, Optical Flow (OF) sensors are often used together with distance sensors such as ultrasonic (sonar) or Light Detection and Ranging (LiDAR) altimeters. Although an OF sensor is not required for basic flight stabilization, it is important for Position Hold and navigation in GNSS-denied environments, such as indoor or underground spaces. In principle, the OF sensor estimates the horizontal velocity of the unmanned aerial vehicle (UAV) by analyzing the movement of visual features in the camera image relative to the ground. The rangefinder provides the corresponding altitude information, which allows these visual measurements to be converted into absolute metric values.

\colger

In autonomen Luftfahrtsystemen werden Optical-Flow-(OF)-Sensoren häufig in Kombination mit Abstandssensoren wie Ultraschall- (Sonar) oder LiDAR-Höhenmessern eingesetzt. Obwohl ein OF-Sensor für die grundlegende Flugstabilisierung nicht erforderlich ist, spielt er eine wichtige Rolle für Position-Hold-Funktionen und die Navigation in GNSS-losen Umgebungen, etwa in Innenräumen oder unterirdischen Bereichen. Grundsätzlich schätzt der OF-Sensor die horizontale Geschwindigkeit des unbemannten Luftfahrzeugs (UAV), indem er die Bewegung von Bildmerkmalen im Kamerabild relativ zum Boden analysiert. Der Entfernungssensor liefert die dazugehörige Höheninformation, wodurch diese visuellen Messungen in absolute metrische Werte umgerechnet werden können.

\coleng

The integration of these sensors is handled through various serial communication protocols, most notably MSP (MultiWii Serial Protocol), MAVLink, and DroneCAN. Which protocol is used depends largely on the flight control firmware that is running on the system:

\colger

Die Integration dieser Sensoren erfolgt über verschiedene serielle Kommunikationsprotokolle, insbesondere MSP (MultiWii Serial Protocol), MAVLink und DroneCAN. Welches Protokoll zum Einsatz kommt, hängt dabei maßgeblich von der verwendeten Firmware des Flugcontrollers ab:

\coleng

\begin{itemize}
\item \textsl{Betaflight}: Historically optimized for high-performance manual flight, Betaflight did not provide native optical flow support in earlier versions. Only with the release of version 4.6 has limited support for integrated positioning sensors been introduced.
\end{itemize}

\colger

\begin{itemize}
\item \textsl{Betaflight}: Betaflight wurde ursprünglich für leistungsorientierten manuellen Flug entwickelt und bot in früheren Versionen keine native Unterstützung für Optical-Flow-Sensoren. Erst mit Version 4.6 wurde eine begrenzte Unterstützung für integrierte Positionssensoren eingeführt.
\end{itemize}
...
\begin{itemize}
\item \textsl{INAV}: Compared to Betaflight, INAV is more navigation-oriented and supports a wider range of positioning sensors. However, compatibility is still largely limited to selected peripherals, primarily MSP-based devices such as the MicoAir MTF-01/MTF-01P (see Figure~\ref{AbbildungMicoAirMFT01P}) or the Matek 3901-L0X (see Figure~\ref{AbbildungMatek3901L0X}).
\end{itemize}

\colger

\begin{itemize}
\item \textsl{INAV}: Im Vergleich zu Betaflight ist INAV stärker auf Navigation ausgelegt und unterstützt eine größere Bandbreite an Positionssensoren. Die Kompatibilität beschränkt sich jedoch weiterhin auf ausgewählte Peripheriegeräte, insbesondere MSP-basierte Sensoren wie den MicoAir MTF-01/MTF-01P (see Figure~\ref{AbbildungMicoAirMFT01P}) oder den Matek 3901-L0X (siehe Abbildung~\ref{AbbildungMatek3901L0X}).
\end{itemize}
...
\begin{itemize}
\item \textsl{ArduPilot}: This firmware provides the most extensive support for sensor fusion. It offers broad support for protocols such as MSP, MAVLink, and the industrial-grade DroneCAN standard, which makes it a common choice for complex autonomous systems and research platforms.
\end{itemize}

\colger

\begin{itemize}
\item \textsl{ArduPilot}: Diese Firmware bietet die umfassendste Unterstützung für Sensorfusion. Sie unterstützt verschiedene Protokolle wie MSP, MAVLink und den industriellen Standard DroneCAN und wird daher häufig für komplexe autonome Systeme und Forschungsplattformen eingesetzt.
\end{itemize}

\coleng

Standard hardware implementations typically use a Universal Asynchronous Receiver–Transmitter (UART) interface. The sensor’s TX (transmit) and RX (receive) pins must be cross-connected to the corresponding RX and TX ports of the flight controller. In addition, a common ground (GND) and a suitable regulated voltage supply (VCC) are required to ensure reliable signal transmission and stable power delivery.

\colger

Standardimplementierungen der Hardware verwenden in der Regel eine UART-Schnittstelle (Universal Asynchronous Receiver–Transmitter). Die TX- (Sende-) und RX- (Empfangs-) Pins des Sensors müssen dabei über Kreuz mit den entsprechenden RX- und TX-Pins des Flugcontrollers verbunden werden. Zusätzlich sind eine gemeinsame Masseleitung (GND) sowie eine geeignete geregelte Spannungsversorgung (VCC) erforderlich, um eine zuverlässige Signalübertragung und eine stabile Stromversorgung sicherzustellen.

\coleng

LiDAR laser rangefinders and optical flow sensors available on the market differ in several aspects, including their field of view, required ambient illumination, minimum and maximum operating height, and the maximum velocity at which reliable optical flow measurements can still be obtained. The sensor MicoAir MTF-01P shown in Figure~\ref{AbbildungMicoAirMFT01P}, for example, has a field of view of 42 degrees. For reliable operation, a minimum ambient illumination of 60~lux is required. Optical flow measurements begin at a height of approximately 8~cm and remain reliable at speeds of up to about 7~m/s at an altitude of one meter. The integrated laser rangefinder operates within a measurement range from 2~cm to 12~m. The Matek 3901-L0X shown in Figure~\ref{AbbildungMatek3901L0X} has similar characteristics, but its measurement range is limited to a maximum of approximately 2~m.

\colger

Die am Markt verfügbaren LiDAR-Laserentfernungsmesser und optischen Flusssensoren unterscheiden sich unter anderem hinsichtlich ihres Blickfelds, der erforderlichen Umgebungshelligkeit, der minimalen und maximalen Einsatzhöhe sowie der maximalen Geschwindigkeit, bei der eine zuverlässige Flussmessung noch möglich ist. Der in Abbildung~\ref{AbbildungMicoAirMFT01P} gezeigte Sensor MicoAir MTF-01P besitzt beispielsweise ein Blickfeld von 42 Grad. Für einen zuverlässigen Betrieb werden mindestens 60~Lux Umgebungshelligkeit benötigt. Die Messung des optischen Flusses beginnt ab einer Höhe von etwa 8~cm und funktioniert bei einer Höhe von einem Meter bis zu einer maximalen Geschwindigkeit von etwa 7~m/s. Der integrierte Laserentfernungsmesser arbeitet in einem Messbereich von 2~cm bis 12~m. Der in Abbildung~\ref{AbbildungMatek3901L0X} gezeigte Matek 3901-L0X weist vergleichbare Eigenschaften auf, allerdings ist sein Messbereich auf maximal etwa 2~m begrenzt.
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{MicoAir_Optical_Flow_Lidar_Sensor_MTF_01P_front.jpg}
    \vspace{0pt} % sorgt für Top-Ausrichtung
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{MicoAir_Optical_Flow_Lidar_Sensor_MTF_01P_side.png}
    \vspace{0pt}
  \end{minipage}
  \caption{MicoAir MTF-01P Optical Flow and Lidar Sensor (33x21x17mm)}
  \label{AbbildungMicoAirMFT01P}
\end{figure}
\FloatBarrier
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Matek_3901_L0X_Optical_Flow_and_Lidar_Front.png}
    \vspace{0pt} % sorgt für Top-Ausrichtung
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Matek_3901_L0X_Optical_Flow_and_Lidar_Back.jpg}
    \vspace{0pt}
  \end{minipage}
  \caption{Matek 3901-L0X Optical Flow and Lidar Sensor}
  \label{AbbildungMatek3901L0X}
\end{figure}
\FloatBarrier
...
ELRS remote controls (transmitters) use ExpressLRS (see section~\ref{AbschnittExpressLRS}) as the firmware for the radio protocol and transmitter module, and EdgeTX (see section~\ref{AbschnittEdgeTX}) as the operating system -- a modernized version of OpenTX. EdgeTX generates the control signals, while ExpressLRS transmits them.

\colger

ELRS-Fernsteuerungen (Sender) verwenden ExpressLRS (siehe Abschnitt~\ref{AbschnittExpressLRS}) als Firmware für das Funkprotokoll und das Sendermodul sowie EdgeTX (siehe Abschnitt~\ref{AbschnittEdgeTX}) als Betriebssystem, eine modernisierte Variante von OpenTX. EdgeTX erzeugt die Steuersignale, und ExpressLRS überträgt sie.
...
The video transmitter (VTX) transmits the live image from the FPV camera to the FPV goggles using the selected channel within the defined frequency band. In addition to analog video transmitters, there are several digital systems that are incompatible with one another. Video transmitters are sold either as standalone devices or as kits (see Figure~\ref{AbbildungDJIO4AirUnit}) that include a camera and antenna(s). The figures~\ref{AbbildungSpeedyBeeTX800FPVVTX} and~\ref{AbbildungFoxeerReaperV2VTX} show two examples of analog video transmitters.

\colger

Der Videosender (VTX) überträgt das Livebild der FPV-Kamera über den eingestellten Kanal im festgelegten Frequenzband an die FPV-Brille. Neben analogen Videosendern existieren auch verschiedene digitale Systeme, die untereinander nicht kompatibel sind. Videosender werden entweder als Einzelgerät oder als Kit (siehe Abbildung~\ref{AbbildungDJIO4AirUnit}) mit Kamera und Antenne(n) verkauft. Die Abbildungen~\ref{AbbildungSpeedyBeeTX800FPVVTX} und~\ref{AbbildungFoxeerReaperV2VTX} zeigen zwei analogen Videosender.
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Foxeer_VTX_5_8G_Reaper_Extrem_V2_2_5_W_Vorderseite.png}
    \vspace{0pt} % sorgt für Top-Ausrichtung
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Foxeer_VTX_5_8G_Reaper_Extrem_V2_2_5_W_Rueckseite.png}
    \vspace{0pt}
  \end{minipage}
  \caption{Foxeer 5.8G Reaper Extreme V2 2.5W Video Transmitter (Front and Back)}
  \label{AbbildungFoxeerReaperV2VTX}
\end{figure}
\FloatBarrier
...
Analog video transmitters are cross-compatible among manufacturers, while this is not the case for digital systems. For digital video transmission, the transmitter and receiver must be from the same manufacturer. Digital systems for live video transmission are available from DJI (see Figure~\ref{AbbildungDJIO4AirUnit}), HDZero, and CaddxFPV under the brand name Walksnail.

\colger

Analoge Videosender sind herstellerübergreifend mit analogen Videobrillen kompatibel. Bei digitalen Systemen ist das nicht der Fall -- hier müssen Sender und Empfänger vom selben Hersteller stammen. Digitale Systeme zur Übertragung des Livebilds werden von DJI (siehe Abbildung~\ref{AbbildungDJIO4AirUnit}), HDZero und CaddxFPV unter dem Namen Walksnail angeboten.
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{DJI_O4_Air_Unit_Pro_front_small.png}
    \vspace{0pt} % sorgt für Top-Ausrichtung
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{DJI_O4_Air_Unit_front_small.png}
    \vspace{0pt}
  \end{minipage}
  \caption{Digital systems for Live Video Transmission, such as the DJI O4 Air Unit Pro (left) and the DJI O4 Air Unit (right), offer excellent Image Quality but typically lack cross-manufacturer Compatibility}
  \label{AbbildungDJIO4AirUnit}
\end{figure}
\FloatBarrier
...
Based on Figures~\ref{AbbildungSpeedyBeeTX800FPVVTX} and~\ref{AbbildungFoxeerReaperV2VTX}, it becomes evident that video transmitters available on the market operate with different supply voltages. When connecting a video transmitter to the flight controller, it is therefore essential to ensure that it is supplied with an appropriate electrical voltage. An incorrect voltage supply may result in improper operation or permanent damage to the video transmitter.

\colger

Anhand der beiden Abbildungen~\ref{AbbildungSpeedyBeeTX800FPVVTX} und~\ref{AbbildungFoxeerReaperV2VTX} wird deutlich, dass am Markt erhältliche Videosender mit unterschiedlichen Betriebsspannungen arbeiten. Beim Anschluss an den Flugcontroller ist daher sicherzustellen, dass der Videosender mit einer geeigneten elektrischen Spannung versorgt wird. Eine falsche Spannungsversorgung kann dazu führen, dass der Videosender nicht ordnungsgemäß funktioniert oder dauerhaft beschädigt wird.
...
\begin{figure}[htb]
  ...
  \caption{The Skyzone SKY04X Pro FPV Goggle provides an A/V Port}
...
\begin{figure}[htb]
  \centering
    \includegraphics[width=\linewidth]{HDZero_BoxPro_Plus_small.png}
  \caption{The HDZero BoxPro+ provides an A/V Port and input and output HDMI Ports}
  \label{AbbildungBrilleHDMIPort}
\end{figure}
\FloatBarrier
